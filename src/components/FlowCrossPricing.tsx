        ...
        <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <div
              ...
              className={`relative p-8 rounded-2xl transition-all duration-500 hover:scale-105 plan-card-enter ${
                plan.name === "Flow+"
                  ? "glass-effect border-2 border-dashed border-primary floating-glow"
                  : plan.popular
              ...
              <div className="relative z-10">
                ...
                <div className="mb-6">
                  <div className="flex items-baseline">
                    <span className="text-4xl font-bold text-foreground transition-all duration-500">
                      {plan.name === "Flow Basic" ? (
                        plan.price
                      ) : (
                        <>
                          $<CountUpAnimation 
                            end={parseFloat((billingPeriod === "monthly" ? plan.monthlyPrice : plan.yearlyPrice).replace('$', ''))} 
                            duration={800}
                          />
                        </>
                      )}
                  ...
                  {billingPeriod === "yearly" && plan.name !== "Flow Basic" && (
                    <p className="text-sm text-green-400 mt-1 transition-opacity duration-300">
                      Экономия <CountUpAnimation 
                        end={parseFloat((plan.name === "Flow+" ? "9.89" : "19.89"))} 
                        duration={600}
                        prefix="$"
                      /> в год
                    </p>
