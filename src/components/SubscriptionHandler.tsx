...
const SubscriptionHandler = ({ planName, planPrice, planFeatures, buttonText, variant }: SubscriptionHandlerProps) => {
  ...
  const handleSubscribe = () => {
    if (planName === "Flow Basic") {
      // Прокрутка к разделу скачивания для бесплатного плана
  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className={`max-w-2xl ${planName.includes("v4") ? "cosmic-bg" : "glass-effect"}`}>
          {planName.includes("v4") && <SpaceBackground />}
          ...
          {!isLoggedIn() ? (
            ...
            <div className="space-y-6 relative z-10">
              {/* Plan Summary */}
              <Card className={planName.includes("v4") ? "bg-white/10 border-white/20" : ""}>
                <CardHeader>
                ...
                <div className="grid gap-4">
                  <Card 
                    className={`cursor-pointer hover:border-primary transition-colors ${
                      planName.includes("v4") ? "bg-white/10 border-white/20 hover:border-primary" : ""
                    }`}
              ...
              {/* User Info */}
              <Card className={`${planName.includes("v4") ? "bg-white/5 border-white/10" : "bg-muted/20"}`}>
